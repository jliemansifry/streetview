...
def load_data():
    ...
    to use different class labels. If the image data tensor ever became too large
    to fit in memory, it would be necessary to load it in smaller batches that could
    then be trained on. 
    '''
...
def process_Xy(X, y, idx_offset, model_params):
    ...
    INPUT:  (1) 4D numpy array: all X
            ...
            (3) integer: idx offset to be used when reading through the X tensor
                Images are loaded such that index 0 is facing North, 1: East, 
                2: South, 3: West, 4: North, etc. 
                This allows the N, E, S, and W image data to be split back 
                out such that unique X will be used for each cardinal direction. 
    ...
    '''
    split_train, split_test = 72000, 7812 # a fractional split of all len(data) 
                                          # results in a float; this is simpler
