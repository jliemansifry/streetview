...
class ColorDescriptor(object):
    ...
    def describe(self, image, norm_and_flatten=True):
        ...
        if self.hist_loc == 'quadrant':
            ...
            for (startX, endX, startY, endY) in segments:
                ...
                cv2.rectangle(image_mask, (startX, startY), (endX, endY),
                              255, -1)
    ...
    def get_hsv_xyz(self, image):
        ...
        OUTPUT: (1) x value
                (2) y
                (3) z
                (4) 2D numpy array of rgb vals for every color

        This is a helper function to convert the HSV values from cylindrical
        space (ie. theta, r, z) to x, y, z for matplotlib 3D scatterplot
        '''
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    ...
    def plot3d_hsv(self, image, image2=None, subsample=200, show_plot=True,
                   base_ax=None, animate=False, alpha=0.7,
                   saveas='you_should_rename_this'):
        '''
